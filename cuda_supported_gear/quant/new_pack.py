...
def triton_quantize_and_pack_along_last_dim_witherror(data: torch.Tensor, group_size: int, bit: int):
    assert len(data.shape) == 4
    shape = data.shape
    B, nh, D, T = shape
    # ================== Get Scale & Zeros ===============
    ...
    grid = lambda meta: (triton.cdiv(quant_data.shape[0], BLOCK_SIZE_N), data.shape[1] // feat_per_int,)
    _pack_along_last_dim[grid](bit, quant_data, code, quant_data.shape[0], 
                                quant_data.shape[1], feat_per_int, 
                                BLOCK_SIZE_N=BLOCK_SIZE_N, 
                                num_warps=8)
    return code.view(B, nh, D, -1), scale.reshape(scale_mn_shape), mn.reshape(scale_mn_shape), error
...
def headwise_lrap(tensor: torch.Tensor, rank, loop):
    ...
    tensor = tensor.float()
    p_base = torch.rand(batch,num_head,head_dim, rank).to(tensor.device)
    q_base = torch.rand(batch,num_head,seq_len, rank).to(tensor.device)
    for i in range(loop):
        if i == loop - 1:
            p_base = torch.linalg.qr(p_base).Q
        q_base = tensor @ p_base[0]
        if i == loop - 1:
            q_base = torch.linalg.qr(q_base).Q
        p_base = torch.transpose(tensor, 2, 3) @ q_base[0]
    # input = q_base[0] @ torch.transpose(p_base[0], 2, 3)
    # input = input.view(batch, dim1, dim2, dim3)

    # input = input.type(dtype)
    p_base = p_base.type(dtype)
    q_base = q_base.type(dtype)
